...
def simulation_input():
    def submit():
        ...
        starting_infected = starting_infected_entry.get()
        ...
        if not (1 <= int(starting_infected) <= 20):
...
class Dot(pygame.sprite.Sprite):
    ...
    def killswitch(self, cycles_to_fate=150, mortality_rate=0.2):
...
class Simulation:
    def __init__(self, width=1000, height=800):
        ...
        self.n_vulnerable = self.total_population
        self.n_infected = self.starting_infected
        self.T = 2000
        self.cycles_to_fate = 180
        self.mortality_rate = mortality_rate
    ...
    def start(self, randomize=False):
        ...
        # Add vulnerable dots
        for _ in range(self.n_vulnerable):
        ...
        for i in range(self.T):
            ...
            collision_group = pygame.sprite.groupcollide(self.vulnerable_container, self.infected_container, True,
                                                         False)
            for dude in collision_group:
                ...
                if just_a_number <= self.infection_rate:
                    ...
                    new_dude.vel *= -1
                    new_dude.killswitch(self.cycles_to_fate, self.mortality_rate)
